...
class DataBase:
    ...
    @classmethod
    async def remove_outdated_indexes(cls, document_class, defined_indexes, existing_indexes):
        ...
        for index_name in indexes_to_remove:
            if index_name not in ["_id_"]:  # Avoid dropping the default index on the _id field
                logger.debug(f"Removing outdated index '{index_name}' from '{collection_name}'.")
        ...
        for field_name, field_instance in document_class._fields.items():
            if getattr(field_instance, 'unique', False):
                ...
                if index_name not in existing_indexes:
                    try:
                        await collection.create_index([(field_name, ASCENDING)], unique=True, name=index_name)
                    ...
                    logger.debug(f"Index '{index_name}' already exists for '{field_name}' in '{collection_name}'.")
        ...
        if hasattr(document_class, "Meta") and hasattr(document_class.Meta, "indexes"):
            ...
            for index in document_class.Meta.indexes:
                try:
                    ...
                    fields = [(f, ASCENDING) if isinstance(f, str) else f for f in fields_input]
                    _options = {k: v for k, v in index.items() if k != "fields"}
                    index_name = _options.get("name", "_".join([str(f[0]) for f in fields]))
                    defined_indexes.add(index_name)  # Add to defined_indexes
                    if index_name not in existing_indexes:
                        logger.debug(f"Creating index for fields = {fields} and options = {_options}")
                        await collection.create_index(fields, **_options)
                    else:
                        logger.debug(f"Index '{index_name}' already exists and won't be recreated.")
                except OperationFailure as e:
                    cls.handle_operation_failure(e, _options)

        await cls.remove_outdated_indexes(document_class, defined_indexes, existing_indexes.keys())
