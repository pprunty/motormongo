    ...
    def __set__(self, obj, value):
        ...
        if isinstance(value, str):
            ...
            if matching_enum is not None:
                ...
                raise ValueError(f"String value '{value}' does not match any member of {self.enum.__name__}")
        ...
        if value is not None and not isinstance(value, self.enum):
            raise ValueError(f"Value for {self.name} must be an instance of {self.enum.__name__} or a matching string")
